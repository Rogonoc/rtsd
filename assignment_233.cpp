...
void *threadFunc(void *pArg) 
{
    ...
    // Initialize signal
    sigemptyset(&set);                             // Initialize "set" signal
    sigaddset(&set, signum);                       // Associate "signum" with the "set" signal
    sigprocmask(SIG_BLOCK, &set, NULL);            // Initially block "set" signal
    ...
    new_time.it_value.tv_nsec = 100000000;      // 0.1 sec
    new_time.it_interval.tv_sec = 0;
    new_time.it_interval.tv_nsec = 100000000;   // 0.1 sec
    ...
    for(i = 0; i < 101; i++)
    {
        ...
        // Trying to let a mode switch occur
